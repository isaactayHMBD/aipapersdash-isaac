...
function addArticleToDOM(article) {
    ...
    let displayAI = article.AIsummary;
    let displayDate = article.date;
    let displayTitle = article.title.length > 100 ? article.title.substr(0, 90) + '...' : article.title;
    let displaySummary = article.summary.length > 300 ? article.summary.substr(0, 300) + '...' : article.summary;
    console.log(displayAI)
    console.log(displayDate)
    console.log(displayTitle)

    if(displayAI == 1){
        articleDiv.classList.add('bg-pink-800', 'shadow-lg', 'rounded-lg', 'overflow-hidden', 'p-4', 'text-white', 'transition-colors', 'duration-200', 'hover:bg-pink-700', 'cursor-pointer');
        console.log('AI 1')
    ...
    articleDiv.innerHTML = `
        <h4 class="text-gray-200">${displayDate}</h4>
